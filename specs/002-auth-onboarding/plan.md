...
**Architecture:** 내부 회원 ID와 제공자 계정 연결을 분리하고 DB 유일 제약으로 중복 가입·연결 경합을 막는다. 서버 세션은 PostgreSQL에 보존하며 보호 요청마다 현재 회원 승인 상태와 프로젝트 배정을 다시 읽는다. 독립 정책을 먼저 구현했고 `feature/app-bootstrap`을 기능 브랜치에 병합한 뒤 데이터·OAuth·세션·화면을 연결했다.
...
- 다른 PC의 앱 골격 변경은 원격에서 검증된 뒤 기능 브랜치에 병합한다.
- 사용자의 untracked `outputs/`는 이 브랜치에서 정리·추가하지 않는다.
...
FR-001/002/010~013은 Tasks 2~3, FR-003/004/009는 Tasks 1/3/5, FR-005~008은 Tasks 2/4가 담당한다. SC-001~005는 Task 6의 자동·실연동 검증으로, SC-006은 내부 팀원 확인으로 판정한다. 실제 제공자 인증과 팀원 사용성 확인은 자동 시험 통과와 구분한다.
